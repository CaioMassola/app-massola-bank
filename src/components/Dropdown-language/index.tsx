...
const DropDownLanguage = (props: IDropDown) => {
    ...
    const find_idx = languages.findIndex((x) => x.surname == language_redux?.surname)
    ...
    return (
        <SelectDropdown
                ...
                return (
                    <View style={styles.viewDropdown}>
                        {selectedItem ? (
                            ...
                            <Icon type='ionicon' name="md-earth-sharp" color={"white"} size={32} tvParallaxProperties />
                        )}
                        <Text
                            style={styles.textLanguage}
                            accessible
                            accessibilityLabel={find_idx >= 0 ? languages[find_idx].language : i18n.t('login.language')}
                        >
                            {find_idx >= 0 ? languages[find_idx].language : i18n.t('login.language')}
                        </Text>
                        <Icon name="chevron-down" type='font-awesome' color={"white"} size={18} tvParallaxProperties />
                    </View>
                );
            }}
            ...
            dropdownBackgroundColor={color_value}
        />
    )
}
